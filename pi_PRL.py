...
class SearchMap(nn.Module):
    ...
    # not in use
    def forward(self, x, candidate_programs):

        v = F.softmax(self.v, dim=0)

        action = torch.zeros_like(candidate_programs.programs[0](x))
        for i in range(len(v)):
            action += v[i] * candidate_programs.programs[i](x)
...
class SearchFusionProgram(nn.Module):
    def __init__(self, depth, domain, beta=0.5):
        super().__init__()

        self.domain = domain
        self.search_map = SearchMap(depth=depth)
        self.fusion_programs = FusionPrograms(depth=depth, **parse_domain(domain), beta=beta)
...
prog = SearchFusionProgram(depth=5, domain=domain, beta=0.25)
