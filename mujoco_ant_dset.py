...
class Mujoco_Ant_Dset(object):
    def __init__(self, expert_data, train_fraction=0.7, traj_limitation=-1, randomize=True):
        traj_data = {}
        traj_data['obs'] = np.array([x[0] for x in expert_data])
        traj_data['acs'] = np.array([x[1] for x in expert_data])
